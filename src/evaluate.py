...
5. Calcula 4 métricas específicas para Bug to User Story (conforme README):
   - Tone Score
...
Critério de aprovação (README): TODAS as 4 métricas >= 0.9 E média >= 0.9
...
def display_results(prompt_name: str, scores: Dict[str, float]) -> bool:
    ...
    print("\nMétricas Bug to User Story (fonte: README):")
    ...
    # Critério do README: TODAS as métricas >= 0.9 E média >= 0.9
    ...
    if passed:
        ...
        if not all_above_threshold:
            failing = [k for k, v in scores.items() if v < 0.9]
